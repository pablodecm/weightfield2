...
void Potentials::SetPitchWidthXY(int dimy, int dimx,double p, double w)
{
  ...
  XMAX=pitch*count;	//la dimensione orizzontale è multiplo intero del pitch
...
{			  
  ...
  for(int j=0;j<(int)count;j++)	// set strips
	...
	{
	  dpot[YMAX-1][j*(int)pitch+(int)pitch/2-(int)width/2+k]=(strips==NTYPE) ? vbias : 0.0;
	  wpot[YMAX-1][j*(int)pitch+(int)pitch/2-(int)width/2+k]=0.0;			
	  
	  if(j==((int)count/2))
	    {
	      fix[YMAX-1][j*(int)pitch+(int)pitch/2-(int)width/2+k]=2;
	      wpot[YMAX-1][j*(int)pitch+(int)pitch/2-(int)width/2+k]=1.0;
	    }
	  else
	    {
	      fix[YMAX-1][j*(int)pitch+(int)pitch/2-(int)width/2+k]=1;			
	    }
	}
    }	
}  
