...
/*
  TRandom3 *n;
  mipcharge= n->Landau(78.37,37.77)*YMAX;
...
void Potentials::SetPitchWidthXY(int dimy, int dimx,double p, double w)
{
  ...
  count = (double)XMAX/pitch;    //count è il numero di strip
  ...
  if (((int)count)%2==0) count++;	//count dev'essere intero e dispari	




   cout << "Actual Pitch = " << pitch << " Width = " << width << " Xmax = " << XMAX<< " YMAX = " << YMAX << endl;
...
{			  
  ...
  //  cout << "Boundary X = " << XMAX << " Y = " << YMAX << endl;
  for (int i=0; i<XMAX; i++)
  ...
  //  cout << XMAX-pitch*count << endl;

  for(int j=0;j<(int)count;j++)	// set strips
    {
      for(int k=0;k<width;k++)
	{
	  dpot[YMAX-1][(int)(XMAX-pitch*count)/2 + j*(int)pitch+(int)pitch/2-(int)width/2+k]=(strips==NTYPE) ? vbias : 0.0;
	  wpot[YMAX-1][(int)(XMAX-pitch*count)/2 + j*(int)pitch+(int)pitch/2-(int)width/2+k]=0.0;			
	  
	  if(j==((int)count/2))
	    {
	      fix[YMAX-1][(int)(XMAX-pitch*count)/2+j*(int)pitch+(int)pitch/2-(int)width/2+k]=2;
	      wpot[YMAX-1][(int)(XMAX-pitch*count)/2+j*(int)pitch+(int)pitch/2-(int)width/2+k]=1.0;
	    }
	  else
	    {
	      fix[YMAX-1][(int)(XMAX-pitch*count)/2+j*(int)pitch+(int)pitch/2-(int)width/2+k]=1;			
	    }
	}
...
{				// method for iterative calculation, see: http://en.wikipedia.org/wiki/Relaxation_%28iterative_method%29
  ...
  while(1)		// calculate until accuracy is reached
	...
	break;
      ...
      //      if(XMAX==finest || XMAX==finest/2+1) err=0.001; // if calculating on finest grid, set accuracy to 0.001
      if(XMAX==finest) err=0.005; // if calculating on finest grid, set accuracy to 0.001
	...
	{		
	  for (int y=1; y<(YMAX-1); y++)
	      ...
	      if (!tempfix[y][x])
		{
		  ...
		  // drift potential. Average over 4 points around it
